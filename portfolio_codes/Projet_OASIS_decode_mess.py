...
def estime_freq2(x, Fe):
    ...
    #print([(x, list(freqs).index(x)) for x in freqs if np.abs(x) > 10], len(freqs))
    ...
    freqs_bruits_completees = np.concatenate([[0 for j in range(int(5*len(x)/Fe))],freqs[int(5*len(x)/Fe):int(485*len(x)/Fe)],[0 for j in range(55)],freqs[int(540*len(x)/Fe):len(x)]])
...
def decode(signal):
    ...
    phrase = ""
    while(debut_symbole+2000<=nb_pt):
        signal_symbol=signal[debut_symbole:debut_symbole+2000]
...
x = np.array([np.sin(2*np.pi*500*(t/2000))+np.sin(2*np.pi*480*(t/2000)) for t in range(50000)])
